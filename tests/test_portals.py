...
def test_portal_status_lists_prepared_portals_without_login(client):
    ...
    assert [a['portal'] for a in status['accounts']] == ['bumeran', 'computrabajo', 'linkedin']
    linkedin = next(a for a in status['accounts'] if a['portal'] == 'linkedin')
    assert 'prohíben' in linkedin['riesgo'] and linkedin['limite_diario'] == portals.PORTAL_LIMITS['linkedin']
    prepared = {p['portal']: p for p in status['prepared']}
    assert set(prepared) == {'indeed', 'hiringroom', 'pandape'}
    assert prepared['indeed']['url'].startswith('https://pe.indeed.com/')
